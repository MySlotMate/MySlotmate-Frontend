...
export default function ExperiencesClient({
  ...
  const { data: fetchedEvents, isLoading } = useListPublicEvents();
  // Server-rendered data keeps the cards in the initial HTML; the client
  // query takes over once it resolves.
  const events = fetchedEvents ?? initialEvents;
  const showLoader = isLoading && initialEvents.length === 0;
